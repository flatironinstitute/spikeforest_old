...
class OutputVisualizer:
    ...
    def viewUnitWaveforms(self, units=None, channels=None):
        ...
        list = []
        for unit in units:
            spikes = self._get_random_spike_waveforms(
                unit=unit, max_num=50, channels=channels)
            item = dict(
    ...
    def viewAutoCorrelograms(self, units=None):
        ...
        list = compute_autocorrelograms(
            self._output_analyzer, units=units, max_dt_msec=50, bin_size_msec=2)
        ...
        for i, A0 in enumerate(list):
            ...
            with plt.rc_context({'axes.edgecolor': 'gray'}):
                ...
                plt.bar(left=A0['bin_edges'][0:-1]*1000,
                        height=A0['bin_counts'], width=wid, color='gray')
                plt.xlabel('dt (msec)')
    ...
    def _get_random_spike_waveforms(self, *, unit, max_num, channels):
        ...
        spikes = self._IX.getRawSnippets(
            center_frames=st[event_indices], snippet_len=100, channel_ids=channels)
    ...
    def plot_spike_shapes(self, *, representative_waveforms=None, average_waveform=None, channel_locations=None, ylim=None, max_representatives=None, color='blue', title=''):
        if average_waveform is None:
            if representative_waveforms is None:
                raise Exception(
                    'You must provide either average_waveform, representative waveforms, or both')
        ...
        if representative_waveforms is not None:
            if max_representatives is not None:
                ...
                if W0.shape[2] > max_representatives:
                    indices = np.random.choice(
                        range(W0.shape[2]), size=max_representatives, replace=False)
            ...
            for m in range(M):
                ...
                for j in range(L):
                    XX[:, m, j] = loc[0]+xvals
                    YY[:, m, j] = loc[1]+representative_waveforms[m,
                                                                  :, j]-representative_waveforms[m, 0, j]
            XX = XX.reshape(T, M*L)
            YY = YY.reshape(T, M*L)
    ...
    def _determine_global_ylim(self, list):
        ...
        for item in list:
            ylim0 = self._get_ylim_for_item(
                ...
                representative_waveforms=item.get(
                    'representative_waveforms', None)
            )
...
def compute_autocorrelogram(times, *, max_dt_tp, bin_size_tp, max_samples=None):
    # number of bins to the left of the origin
    ...
    # These are the events we are going to consider
    candidate_inds = np.arange(L)
    if max_samples is not None:
        if len(candidate_inds) > max_samples:
            candidate_inds = np.random.choice(
                candidate_inds, size=max_samples, replace=False)
    vals_list = []  # A list of all offsets we have accumulated
    while True:
        # we only consider events that are within workable range
        candidate_inds = candidate_inds[candidate_inds+step < L]
        # we only consider event-pairs that are within max_dt_tp apart
        candidate_inds = candidate_inds[times2[candidate_inds +
                                               step]-times2[candidate_inds] <= max_dt_tp]
    ...
    # a trick to make the histogram symmetric due to differences in rounding for positive and negative, i suppose
    all_vals = np.sign(all_vals)*(np.abs(all_vals)-bin_size_tp*0.00001)
...
def compute_autocorrelograms(output_analyzer, *, units=None, max_dt_msec=50, bin_size_msec=2, max_samples=5000):
    ...
    for unit in units:
        ...
        bin_counts, bin_edges = compute_autocorrelogram(
            times, max_dt_tp=max_dt_tp, bin_size_tp=bin_size_tp, max_samples=max_samples)
        A = dict(
            unit=unit,
