...
def gen_synth_datasets(datasets,*,outdir,samplerate=32000):
    ...
    for ds in datasets:
        ...
        OX=NeoSpikeTrainsOutputExtractor(spiketrains=spiketrains,samplerate=samplerate)
...
def gen_recording(*,templates,output_extractor,noise_level,samplerate,duration):
    ...
    template_inds=np.random.choice(range(templates0.shape[0]),K,replace=False)
    templates0=templates0[template_inds,:,:]
    upsample_factor=13
    templates0_upsampled=signal.resample_poly(templates0, up=upsample_factor, down=1, axis=2)
    ...
    X=synthesize_timeseries(output_extractor=OX,waveforms=waveforms0,waveforms_tcenter=waveforms_tcenter,samplerate=samplerate,duration=duration,waveform_upsamplefac=upsample_factor,noise_level=noise_level)
    ...
    geom = mea_.positions[:M,1:3] # make sure the geom matches the dimension of X
...
class NeoSpikeTrainsOutputExtractor(si.SortingExtractor):
    ...
    def getUnitSpikeTrain(self, unit_id, start_frame=None, end_frame=None):
        ...
        if self._spiketrains is None:
            self._initialize()
        times = (self._spiketrains[unit_id].times.rescale('s') * self._fs).magnitude
