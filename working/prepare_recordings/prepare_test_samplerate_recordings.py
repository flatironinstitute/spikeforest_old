...
group_name = 'test_samplerate'


def prepare_test_samplerate_studies(*, basedir):
    study_sets = [
        load_study_set_from_md('descriptions/spf_test_samplerate.md')
    ]
    study_set_name = study_sets[0]['name']

    study_set_dir0 = basedir + '/test_samplerate'
    ...
    # study_set_dir = study_set_dir + '.test_samplerate'
    print('Using study set dir: ' + study_set_dir)
    studies = []
    recordings = []
    names = ['sr15000', 'sr20000', 'sr30000', 'sr40000', 'sr60000']
    # description = mt.loadText(path=study_set_dir + '/readme.txt')
    for name in names:
        print('PREPARING: ' + name)
        study_name = 'test_samplerate_' + name
...
# Prepare the studies
studies, recordings, study_sets = prepare_test_samplerate_studies(basedir=basedir)
