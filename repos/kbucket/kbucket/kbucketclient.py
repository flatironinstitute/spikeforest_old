...
class KBucketClient():
    ...
    def setConfig(self, *,
        ...
        if upload_share_id:
            if not upload_token:
                raise Exception(
                    'Cannot set upload_share_id without upload token')
    ...
    def testSaveRemote(self):
        ...
        try:
            self.saveObject({'test': 'upload'}, key={
                            'test': 'upload'}, remote=True)
        except:
            raise Exception('Upload failed.')
        print('Test upload successful.')

    def findFile(self, path=None, *, sha1=None, share_ids=None, key=None, collection=None, local=None, remote=None):
        path, sha1, size = self._find_file_helper(
            path=path, sha1=sha1, share_ids=share_ids, key=key, collection=collection, local=local, remote=remote)
        return path
    ...
    def _save_file_helper(self, path, share_id=None, upload_token=None, basename=None, remote=None, confirm=False):
        ...
        url_check = server_url+url_check_path0+'?signature=' + \
