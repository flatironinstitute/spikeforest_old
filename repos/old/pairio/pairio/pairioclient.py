...
class PairioClient():
    def __init__(self):
        self._config = dict(
            ...
            # where the remote collections live
            url=os.getenv('PAIRIO_URL', 'https://pairio.org:10443'),
            local_database_path=os.getenv(
                'PAIRIO_DATABASE_DIR', _get_default_local_db_fname()),  # for local pairs
        ...
        if user:
            ...
            try:
                if not self.setRemote(test_key, test_val):
                    raise Exception(
                        'Error setting key to remote pairio database.')
            except:
            ...
            if str2 != test_val:
                ...
                raise Exception(
                    'pairio test failed for user={}. {}<>{}'.format(user, str2, test_val))
            else:
                self.setRemote(test_key, None)
    ...
    def getLocal(self, key, return_collection=None):
        if not self._config['local_database_path']:
            raise Exception(
                'Cannot read from local database because local_database_path has not been set')
    ...
    def getRemote(self, key, *, collection=None):
        if not self._config['user']:
            raise Exception(
                'Cannot read from remote database because user has not been set')
    ...
    def set(
        ...
        if remote and user:
            if not url:
                raise Exception(
                    'Cannot set value to remote because no url has been set')
            if not token:
                raise Exception(
                    'Cannot set value to remote because pairio token has not been set')
            if value:
    ...
    def getObject(
        ...
        val = self.get(key=key, collection=collection, local=local,
                       remote=remote, collections=collections)
        if not val:
            return val
    ...
    def setLocal(self, key, value, overwrite=None):
        if not self._config['local_database_path']:
            raise Exception(
                'Cannot write to local database because local_database_path has not been set')
    ...
    def setRemote(self, key, value, overwrite=None):
        if not self._config['user']:
            raise Exception(
                'Cannot write to remote database because user has not been set')
