...
class SFBrowserSnr(vd.Component):
    def __init__(self, output_id):
        ...
        self._snr_threshold_input = vd.components.LineEdit(
            value=8, dtype=float, style=dict(width='70px'))
    ...
    def _update_accuracy_table(self):
        snr_threshold = self._snr_threshold_input.value()
    ...
    def _get_accuracy_table_data(self, *, snr_threshold):
        SF = self._SF_data
        accuracy_table = []
        sorters = set()
        for sname in SF.studyNames():
            ...
            results = _get_study_sorting_results(study)
            tmp = dict(
            ...
            for sorter in results:
                ...
                accuracies1 = accuracies[snrs >= snr_threshold]
                if (len(accuracies1) > 0):
                    text = '{:0.3f}'.format(np.mean(accuracies1))
                else:
                    text='na'                  
    ...
    def render(self):
        return vd.div(
            vd.table(
                vd.tr(
                    vd.td('SNR threshold:'),
                    vd.td(self._snr_threshold_input),
