...
class SFBrowser(vd.Component):
  ...
  def _update_accuracy_table(self):
    ...
    self._accuracy_table_data, self._sorters=self._get_accuracy_table_data(accuracy_threshold=accuracy_threshold)
    self._accuracy_table=self._to_table(self._accuracy_table_data,['study']+self._sorters)    
    self.refresh()
  ...
  def _get_accuracy_table_data(self,*,accuracy_threshold):
    ...
    for sname in SF.studyNames():
        ...
        for sorter in b:
            ...
            accuracies=a['num_matches']/(a['num_matches']+a['num_false_positives']+a['num_false_negatives'])
            tmp[sorter]=dict(
                text=str(np.count_nonzero(accuracies>=accuracy_threshold)),
                callback=lambda sorter=sorter,study=sname: self._open_study_sorter_fig(sorter=sorter,study=study)
            )
