...
def _compute_average_waveform(recording,sorting,*,unit,max_num=50,channels=None,snippet_len=100):
  snippets=_get_random_spike_waveforms(recording=recording,sorting=sorting,unit=unit,max_num=max_num,channels=channels,snippet_len=snippet_len)
...
class VIEW_SelectedTrueUnitComparison(vd.Component):
  ...
  def __init__(self,context):
    ...
    if len(ids)!=1:
      self._error_message='You must select exactly one true unit ({} selected).'.format(len(ids))
      return
    ...
    if self._true_unit_info is None:
      self._error_message='Unable to find info for true unit {}'.format(self._true_unit_id)
      return
    comparison_info=self._context.sorting_result.comparisonWithTruth(format='json')
    ...
    if self._true_unit_comparison_info is None:
      self._error_message='Unable to find comparison info for true unit {}'.format(self._true_unit_id)
      return
    ...
    sx_unmatched_true=get_unmatched_sorting(sx_true,sx_sorted,[self._true_unit_id],[self._sorted_unit_id])
    sx_unmatched_sorted=get_unmatched_sorting(sx_sorted,sx_true,[self._sorted_unit_id],[self._true_unit_id])
    snippet_len=80
    average_waveform=_compute_average_waveform(rx,sx_true,unit=self._true_unit_id,max_num=50,snippet_len=snippet_len)
    args=dict(
      recording=rx,
    ...
    self._true_unit_waveform_widget=UnitWaveformWidget(**args,sorting=sx_true,unit_id=self._true_unit_id)
    self._sorted_unit_waveform_widget=UnitWaveformWidget(**args,sorting=sx_sorted,unit_id=self._sorted_unit_id)
    self._unmatched_true_unit_waveform_widget=UnitWaveformWidget(**args,sorting=sx_unmatched_true,unit_id=self._true_unit_id)
    self._unmatched_sorted_unit_waveform_widget=UnitWaveformWidget(**args,sorting=sx_unmatched_sorted,unit_id=self._sorted_unit_id)
  ...
  def render(self):
    ...
    div=vd.div(
        vd.h3('True unit {}, Sorted unit {}'.format(self._true_unit_id,self._sorted_unit_id)),
        vd.div(boxes)
...
class VIEW_SelectedTrueUnitComparison2(vd.Component):
  ...
  def __init__(self,context):
    ...
    snippets1=_get_random_spike_waveforms(recording=rx, sorting=sx_true, unit=self._true_unit_id, max_num=250, snippet_len=snippet_len)
    snippets2=_get_random_spike_waveforms(recording=rx, sorting=sx_unmatched_true, unit=self._true_unit_id, max_num=250, snippet_len=snippet_len)
    snippets3=_get_random_spike_waveforms(recording=rx, sorting=sx_unmatched_sorted, unit=self._sorted_unit_id, max_num=250, snippet_len=snippet_len)
    components=_compute_principal_components(snippets1.reshape((snippets1.shape[0]*snippets1.shape[1],snippets1.shape[2])),num_components=2)
    features1=components.transpose() @ snippets1.reshape((snippets1.shape[0]*snippets1.shape[1],snippets1.shape[2]))
    features2=components.transpose() @ snippets2.reshape((snippets2.shape[0]*snippets2.shape[1],snippets2.shape[2]))
    features3=components.transpose() @ snippets3.reshape((snippets3.shape[0]*snippets3.shape[1],snippets3.shape[2]))
    features=np.concatenate((features1,features2,features3),axis=1)
    labels=np.concatenate((1*np.ones(features1.shape[1]),2*np.ones(features2.shape[1]),3*np.ones(features3.shape[1])))
    ...
    class _ClusterPlot(vd.components.Pyplot):
      def __init__(self,*,features,labels):
        ...
        self._labels=labels
      def plot(self):
        plt.scatter(self._features[0,:],self._features[1,:],marker='.',c=self._labels)
...
class ControlPanel(vd.Component):
  def __init__(self,context):
    ...
    general_view_classes=[VIEW_GeneralInfo,VIEW_Timeseries,VIEW_TrueUnitsTable]
    truth_view_classes=[VIEW_TrueUnitWaveforms,VIEW_TrueAutocorrelograms]
    sorting_view_classes=[VIEW_UnitWaveforms,VIEW_Autocorrelograms]
    comparison_view_classes=[VIEW_SelectedTrueUnitComparison,VIEW_SelectedTrueUnitComparison2,VIEW_SelectedTrueUnitComparison3]

    self._general_view_launcher=ViewLauncher(context,general_view_classes)
    self._truth_view_launcher=ViewLauncher(context,truth_view_classes)
    self._sorting_view_launcher=ViewLauncher(context,sorting_view_classes)
    self._comparison_view_launcher=ViewLauncher(context,comparison_view_classes)
...
class SortingResultExplorer(vd.Component):
  ...
  def render(self):
    ...
    W_CP=Container(self._control_panel,position=(0,0),size=(width1,height),style=style0)
    self._view_container_north.setSize((width2,height1))
    self._view_container_south.setSize((width2,height2))
    W_VCN=Container(self._view_container_north,position=(width1+10,0),size=(width2,height1),style=style0)
    W_VCS=Container(self._view_container_south,position=(width1+10,height1+10),size=(width2,height2),style=style0)
...
class VIEW_SelectedTrueUnitComparison3(vd.Component):
  ...
  def __init__(self,context):
    ...
    self._true_unit_waveform_widget=UnitWaveformWidget2(**args,sorting=sx_true,unit_id=self._true_unit_id)
    self._sorted_unit_waveform_widget=UnitWaveformWidget2(**args,sorting=sx_sorted,unit_id=self._sorted_unit_id)
    self._unmatched_true_unit_waveform_widget=UnitWaveformWidget2(**args,sorting=sx_unmatched_true,unit_id=self._true_unit_id)
    self._unmatched_sorted_unit_waveform_widget=UnitWaveformWidget2(**args,sorting=sx_unmatched_sorted,unit_id=self._sorted_unit_id)
