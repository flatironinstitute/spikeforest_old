...
class MainWindow(vd.Component):
  ...
  def render(self):
    ...
    select_table=vd.table(rows,style={'text-align':'left','width':'auto'},class_='table')
    return vd.div(
      select_table,
...
def main():
  ...
  if os.environ.get('SPIKEFOREST_PASSWORD',None):
    ...
    sf.kbucketConfigRemote(name='spikeforest1-readwrite',password=os.environ.get('SPIKEFOREST_PASSWORD'))
  else:
